...
class FlopsCounter:
    ...
    def estimate_flops(self, batch_seqlens, delta_time, **kargs):
        ...
        promised_flops = get_device_flops(device_name=kargs["device"])
...
# flops_counter = FlopsCounter(config)
# estimated_flops, promised_flops = flops_counter.estimate_flops([4096], 0.5)
# print(f"Estimated FLOPS: {estimated_flops} TFLOPS")
# print(f"Promised FLOPS: {promised_flops} TFLOPS")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="CPU",
        choices=list(_DEVICE_FLOPS.keys()),
        help="Specify which device to run the model on.",
    )
...
if __name__ == "__main__":
    ...
    print(f"Batch sequence length: {args.batch_seqlens}")

    config = AutoConfig.from_pretrained(args.model)
    flops_counter = FlopsCounter(config)
    estimated_flops, promised_flops = flops_counter.estimate_flops(
        args.batch_seqlens,
        0.5,
        device=args.device
    )
    ...
    print(f"Estimated FLOPs: {estimated_flops}TFLOPs")
    print(f"Promised FLOPs: {promised_flops}TFLOPs")
    print(f"MFU: {estimated_flops / promised_flops * 100:.2f}%")